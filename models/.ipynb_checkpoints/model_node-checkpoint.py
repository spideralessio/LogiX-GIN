...
class GIN(torch.nn.Module):
    ...
    def forward(self, x, edge_index, tau=1, deterministic=False):
        xs = []
    ...
    def forward_e(self, x, edge_index, tau=1, deterministic=False):
        ret_x = []
        ret_y = []
        xs = []
        for conv in self.convs:
...
class GINTELL(torch.nn.Module):
    def __init__(self, num_features, num_classes, num_layers=5, hidden_dim=64, dropout=0.1):
        ...
        for i in range(num_layers):
            ...
            self.convs.append(conv)
        self.fc = LogicalLayer(2*num_layers*hidden_dim, num_classes, dummy_phi_in=False)
    
        self.dropout = torch.nn.Dropout(dropout)


    def forward(self, x, edge_index, discrete=False, *args, **kwargs):
        xs = []
